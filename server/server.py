...
def mintBAT():
    ...
    w3 = Web3(HTTPProvider('https://sepolia-rollup.arbitrum.io/rpc'))

    contract = w3.eth.contract(address=0x528268f80FE98669D712f524aA4a66141218bbc5, abi=[
	{
		"inputs": [
			{
    ...
    try:
        # Call the generate_certificate function in your smart contract
        transaction = contract.functions.mintBAT(analysis_text).transact({'from': 0xF6b058eC45018dB4BcE04848072baA7794ce5784})
        transaction_receipt = w3.eth.waitForTransactionReceipt(transaction)
        if transaction_receipt['status'] == 1:
            return jsonify({"message": "Certificate minted successfully."})
        else:
            return jsonify({"message": "Certificate minting failed."})
